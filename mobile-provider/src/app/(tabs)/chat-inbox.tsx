/**
 * ChatInbox — Provider app.
 *
 * Lists every active conversation the current provider is a participant
 ...
 * route parameters.
 *
 * Design: matches the provider app's cyan-led dark palette
 * (``#1a1a1a`` / ``#0f172a`` background, cyan ``#00bfff`` accent).
 */
...
import { providerAPI, type ChatInboxConversation } from '@/services/providerAPI';

export default function ChatInboxScreen() {
  const router = useRouter();

  const [conversations, setConversations] = useState<ChatInboxConversation[]>([]);
  ...
  const fetchInbox = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const data = await providerAPI.getChatInbox('provider', 'active');
  ...
  const handleOpenConversation = useCallback(
    (conv: ChatInboxConversation) => {
      ...
      // The chat screen is at the root Stack level so it covers the tab bar.
      router.push({
        pathname: '/chat',
        params: { booking_id: conv.booking_id },
      });
    },
    [router],
  );
  ...
  const renderItem = useCallback(
    ({ item }: { item: ChatInboxConversation }) => (
    ...
    <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Negotiate and confirm with your customers.
        </Text>
      </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyWrap}>
              ...
              <Text style={styles.emptyBody}>
                Once you accept a customer&apos;s booking, your chat will appear here.
              </Text>
            </View>
          }
        />
...
interface RowProps {
  conversation: ChatInboxConversation;
  onPress: (conv: ChatInboxConversation) => void;
}

const ConversationRow: React.FC<RowProps> = ({ conversation, onPress }) => {
  // Provider sees the customer's name (counterpart).
  const counterpartName = conversation.customer_name || 'Customer';
    ...
    >
      ...
      <View style={styles.rowBody}>
        ...
        <View style={styles.serviceTagPill}>
          <Text style={styles.serviceTagIcon}>🛠️</Text>
...
const styles = StyleSheet.create({
  ...
  // Inline empty state (provider app doesn't have a shared EmptyState)
  emptyWrap: {
    alignItems: 'center',
    paddingHorizontal: Spacing.four,
