...
function RootLayoutNav() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        try {
          ...
          // Redirect to the Dashboard tab if in auth or on initial load
          if (currentSegments[0] === 'auth' || !currentSegments[0]) {
            // Type assertion: the (tabs) group route is generated by
            // Expo Router but the static type union doesn't auto-include
            // group routes, so we narrow the string here.
  ...
  return (
    ...
    >
      ...
      {/* Bottom tab group: Dashboard | Jobs | Chat | Profile */}
      <Stack.Screen name="(tabs)" options={{ animation: 'default' }} />

      {/* Root-level routes that cover the tab bar (modal / detail screens) */}
